...
class GroupNotificationPath extends NotificationPath {
  ...
  @override
  List<String> _getNotificationPath() {
    return ['root', 'chats', 'group', groupId.uuid];
  }
}
...
class ThreadNotificationPath extends NotificationPath {
  ...
  @override
  List<String> _getNotificationPath() {
    return ['root', 'chats', 'group', groupId.uuid, threadId.uuid];
  }
}
...
class DmNotificationPath extends NotificationPath {
  ...
  @override
  List<String> _getNotificationPath() {
    return ['root', 'chats', 'dm', dmId.uuid];
  }
}

class AllChatsNotificationPath extends NotificationPath {
  const AllChatsNotificationPath();
...
class NotificationContainer extends ChangeNotifier {
  ...
  Future<void> clear(NotificationPath path) =>
      _updatePath(path, (deletedNotif) async {
        await Future.wait(deletedNotif
            .map((e) => _localNotificationsPlugin.cancel(e.uuid.hashCode)));
        return [];
      });
  ...
  Future<void> _updatePath(NotificationPath path,
      FutureOr<List<UuidType>> Function(List<UuidType> old) update) async {
    ...
    parentMap.add(lastPathNode, Either.first(await update(prev ?? [])));
